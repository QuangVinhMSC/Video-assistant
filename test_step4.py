...
from services.vector_store import _index
...
def _make_ready_job(tmp_path: Path, job_id: str = "testjob001") -> str:
    """Insert a ready job into the store with files on disk."""
    ...
    from services.job_store import _jobs
    _jobs[job_id] = job

    _index[job_id] = [
        {**FAKE_CHUNKS[0], "embedding": FAKE_EMBEDDING}
    ]
    return job_id
...
def test_ask_job_not_ready(tmp_path):
    ...
    from services.job_store import _jobs
    _jobs["processingjob"] = JobState(
        job_id="processingjob", status=JobStatus.transcribing,
        transcript_txt_path=str(txt_path),
    )
    r = client.post("/ask/processingjob", json={"question": "test"})
...
def test_ask_returns_required_fields(tmp_path):
    job_id = _make_ready_job(tmp_path, "job_tc03")
...
def test_ask_response_structure(tmp_path):
    job_id = _make_ready_job(tmp_path, "job_tc04")
    r = _patched_qa(_ask, job_id)
    body = r.json()
...
def test_used_chunks_propagated(tmp_path):
    job_id = _make_ready_job(tmp_path, "job_tc05")
    r = _patched_qa(_ask, job_id)
    body = r.json()
...
def test_web_search_triggered_when_needed(tmp_path):
    job_id = _make_ready_job(tmp_path, "job_tc06")
...
def test_web_search_not_called_when_not_needed(tmp_path):
    job_id = _make_ready_job(tmp_path, "job_tc07")
...
def test_search_failure_is_silent(tmp_path):
    job_id = _make_ready_job(tmp_path, "job_tc08")
...
def test_llm_failure_returns_500(tmp_path):
    job_id = _make_ready_job(tmp_path, "job_tc09")
...
def test_searcher_returns_empty_on_failure():
    ...
    with patch("services.searcher.DDGS", side_effect=Exception("blocked")) as mock_ddgs:
